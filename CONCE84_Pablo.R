...
#reading the data
setwd("C:/Users/Erick/OneDrive/GIC/ENIGH_1984")
Conc<-read.dbf("CONCE89.dbf",as.is = T)
...
#Adding the values of the expansion factor. The sum is 34 million, which is the number of households in the country.
tot_hogares<-sum(Conc$HOG,to.data.frame=TRUE)
...
Conc$ACUMULA<-cumsum(Conc$HOG) #aqu? creamos una variable de suma acumulada del HOG de viviendas.
...
for(i in 1:99)
{
  a1<-Conc[dim(Conc[Conc$ACUMULA<tam_dec*i,])[1]+1,]$HOG
  Conc<-rbind(Conc[1:(dim(Conc[Conc$ACUMULA<tam_dec*i,])[1]+1),],
              Conc[(dim(Conc[Conc$ACUMULA<tam_dec*i,])[1]+1):dim(Conc[1])[1],])
  b1<-tam_dec*i-Conc[dim(Conc[Conc$ACUMULA<tam_dec*i,])[1],]$ACUMULA
  Conc[(dim(Conc[Conc$ACUMULA<tam_dec*i,])[1]+1),]$HOG<-b1
  Conc[(dim(Conc[Conc$ACUMULA<tam_dec*i,])[1]+2),]$HOG<-(a1-b1)
}

#aqu? estamos creando otra variable de suma acumulada del n?mero de hogares
Conc$ACUMULA2<-cumsum(Conc$HOG)
...
# TOTAL HOGARES
x<-tapply(Conc$HOG,Conc$Nhog,sum)
# DECILES
y<-tapply(Conc$HOG,Conc$DECIL,sum)
# se calcula el promedio (ingreso entre los hogares) tanto para el total como para cada uno de los deciles
ing_cormed_t<-tapply(Conc$HOG*Conc$INGCOR,Conc$Nhog,sum)/x
ing_cormed_d<-tapply(Conc$HOG*Conc$INGCOR,Conc$DECIL,sum)/y
########################## C U A D R O S 
# guardamos los resultados en un data frame
prom_rub <- data.frame (c(ing_cormed_t,ing_cormed_d))
# agregamos el nombre a las filas
...
write.dbf(Conc,file="Conc89.dbf")

write_xlsx(prom_rub,"C:/Users/Erick/OneDrive/GIC/ENIGH_1984/percentiles_89.xlsx")

rm(list=ls())

###################  Creacion de la base 1992 #####################
...
#reading the data
setwd("C:/Users/Erick/OneDrive/GIC/ENIGH_1984")
Conc<-read.dbf("CONCE92.dbf",as.is = T)
...
write.dbf(Conc,file="Conc92.dbf")

write_xlsx(prom_rub,"C:/Users/Erick/OneDrive/GIC/ENIGH_1984/percentiles_92.xlsx")

rm(list=ls())

###################  Creacion de la base 1994 #####################
...
#reading the data
#setwd("C:/Users/Erick/OneDrive/GIC/ENIGH_1984")
Conc<-read.dbf(choose.files(),as.is = T)
...
write.dbf(Conc,file="Conc94.dbf")

write_xlsx(prom_rub,"percentiles_94.xlsx")
...
write.dbf(Conc,file="Conc98.dbf")

write_xlsx(prom_rub,"percentiles_98.xlsx")

rm(list=ls())

################### Creación de la base 2002 ################
library(foreign)
library(survey)
library(doBy)
library(reldist)
...
write.dbf(Conc,file="Conc02.dbf")

write_xlsx(prom_rub,"percentiles_02.xlsx")
